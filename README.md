...
A fast, accessible, single-page portfolio built with vanilla HTML, CSS, and JavaScript — no frameworks, no build tools, just optimised performance. Dark mode, scroll animations, and a responsive layout for all screen sizes.
...
**Skills shown on site:**
`HTML` `CSS` `JavaScript` `TypeScript` `React` `React Native` `Next.js` `Node.js` `NestJS` `PHP` `Laravel` `Svelte` `Docker` `MongoDB` `MySQL` `SQLite` `Supabase` `Python` `Git` `Tailwind CSS`
...
```bash
git clone https://github.com/adilth/portfolio.git
cd portfolio
# Open index.html in your browser, or use a local server:
npx serve .
```

**[⬆ back to top](#adil-tahiri--portfolio)**
